...
def get_dataloaders(train_img_dir, val_img_dir, batch_size=16):
    transform = A.Compose([
        A.Resize(256, 256),
    ...
    train_dataset = SubtitleDataset(train_img_dir, transform=transform)
    val_dataset = SubtitleDataset(val_img_dir, transform=transform)

    if find_hyper:
        val_dataset = torch.utils.data.Subset(val_dataset, list(range(min(300, len(val_dataset)))))
